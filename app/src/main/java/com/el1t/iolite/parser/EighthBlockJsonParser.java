...
public class EighthBlockJsonParser {
	...
	public static ArrayList<EighthBlockItem> parse(InputStream in) throws JSONException, ParseException {
		return parse(Utils.inputStreamToJSON(in));
	}

	public static ArrayList<EighthBlockItem> parse(JSONObject item) throws JSONException, ParseException {
		final JSONArray results = item.getJSONArray("results");
		final ArrayList<EighthBlockItem> blocks = new ArrayList<>(item.getInt("count"));
		JSONObject block;

		for (int i = 0; i < results.length(); i++) {
			block = (JSONObject) results.get(i);
			blocks.add(new EighthBlockItem.ItemBuilder()
